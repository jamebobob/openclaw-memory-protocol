...
2. Count lines in memory/observations.md. Flag if over 100.
3. Does today'"'"'s memory/chat-YYYY-MM-DD.md exist? Flag if missing.
4. Run `openclaw cron list`. Flag any jobs in error status.
5. Check memory/ for files not modified in 7+ days (excluding chat transcripts and daily logs older than 7 days). Flag as stale.
...
8. Run `wc -c ~/.openclaw/workspace/*.md`. Flag if any single file exceeds 18000 characters or total exceeds 140000 characters. Silent bootstrap truncation causes invisible memory loss.
